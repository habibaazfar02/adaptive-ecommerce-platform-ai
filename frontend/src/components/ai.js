...
export async function addAiScores(products) {
  ...
  return tf.tidy(() => {
    ...
    const prices = products.map(p => Number(p.currentPrice || p.current_price || p.basePrice || 0));

    const stockT = tf.tensor1d(stocks);
    const priceT = tf.tensor1d(prices);

    // AI Logic: High Stock + Low Price = Better Recommendation
    ...
    return products.map((p, i) => ({
      ...p,
      aiScore: scores[i]
    })).sort((a, b) => b.aiScore - a.aiScore); // Rank items
  });
}